...
public static class ValidatorFactory {
    ...
    public static IConnector<CollectionValidator<TSubject>> Create<TSubject>(ICollection<TSubject?>? subject, string source, Func<TSubject?, ITerminator> validateItem, ValidatorMode mode = ValidatorMode.And) {
        var validator = new CollectionValidator<TSubject>(subject, source, mode);
        return validator.IsNotNull().And().Each(validateItem);
    }
