...
public interface IElementUpdater {
    ...
    public interface ISetter {
        ...
        IActionConnector Be<TValue>(Func<IElement, TValue> getValueFrom);

        IActionConnector IncreaseBy<TValue>(TValue bonus) where TValue : IAdditionOperators<TValue, TValue, TValue>;
        IActionConnector IncreaseBy<TValue>(Func<IElement, TValue> getBonusFrom) where TValue : IAdditionOperators<TValue, TValue, TValue>;
        IActionConnector DecreaseBy<TValue>(TValue bonus) where TValue : ISubtractionOperators<TValue, TValue, TValue>;
        IActionConnector DecreaseBy<TValue>(Func<IElement, TValue> getBonusFrom) where TValue : ISubtractionOperators<TValue, TValue, TValue>;

        IActionConnector Have<TKey, TValue>(TKey key, TValue value) where TKey : notnull;
        IActionConnector Have<TKey, TValue>(TKey key, Func<IElement, TValue> getValueFrom) where TKey : notnull;
        IActionConnector Have<TKey, TValue>(IEnumerable<KeyValuePair<TKey, TValue>> items) where TKey : notnull;
        IActionConnector Have<TKey, TValue>(Func<IElement, IEnumerable<KeyValuePair<TKey, TValue>>> getItemsFrom) where TKey : notnull;

        IActionConnector Have<TValue>(TValue item);
        IActionConnector Have<TValue>(Func<IElement, TValue> getItemFrom);
        IActionConnector Have<TValue>(params TValue[] items);
        IActionConnector Have<TValue>(Func<IElement, IEnumerable<TValue>> getItemsFrom);
    }
