...
internal static class ElementExtensions {
    ...
    public static TEntity Clone<TEntity>(this TEntity source, IElement? parent = null)
        ...
        if (source is not Element s || target is not Element t) return (TEntity)target;
    ...
    public static void ConfigureModifiers(this IElement element, Action<IFluentBuilder> configure)
        => configure(AttributeEffectsBuilder.For(element));

    public static TElement Get<TElement>(this IRuleSet ruleSet, string name) where TElement : Element
