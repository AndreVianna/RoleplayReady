...
public sealed class IsEqualToCommand
    ...
    private static bool IsEquivalent(object v, object? o) {
        if (v.Equals(o))
            return true;
        if (v is not ICollection vc)
            return false;
        if (o is not ICollection oc)
            return false;
        if (oc.Count != vc.Count)
            return false;
    ...
    private static Dictionary<object, int> GroupByCount(ICollection oc) {
        var d1 = new Dictionary<object, int>();
        foreach (var i in oc) if (!d1.TryAdd(i, 1)) d1[i]++;
        return d1;
    }
}
