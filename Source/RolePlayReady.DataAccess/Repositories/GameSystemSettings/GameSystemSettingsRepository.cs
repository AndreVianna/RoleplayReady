...
public class GameSystemSettingsRepository : IGameSystemSettingsRepository {
    ...
    public async Task<ResultOf<IEnumerable<GameSystemSetting>>> GetManyAsync(string owner, CancellationToken cancellation = default) {
    ...
    public async Task<ResultOf<GameSystemSetting>> GetByIdAsync(string owner, Guid id, CancellationToken cancellation = default) {
    ...
    public async Task<ResultOf<GameSystemSetting>> InsertAsync(string owner, GameSystemSetting input, CancellationToken cancellation = default) {
    ...
    public async Task<ResultOf<GameSystemSetting>> UpdateAsync(string owner, GameSystemSetting input, CancellationToken cancellation = default) {
        var result = await _files.UpsertAsync(owner, string.Empty, input.Id.ToString(), input.Map(), cancellation);
        return input with { Timestamp = result.Value };
    }

    public ResultOf<bool> Delete(string owner, Guid id)
