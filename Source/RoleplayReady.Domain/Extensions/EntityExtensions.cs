...
{
    public static ElementAttribute<T?>? GetAttribute<T>(this IHasAttributes entity, string name)
        => entity.Attributes.OfType<ElementAttribute<T?>>().FirstOrDefault(p => p.Name == name);
    ...
    public static IHasFeatures CloneFeaturesFrom(this IHasFeatures target, IHasFeatures source, params string[] excluding) {
        ...
        foreach (var feature in source.Features.Where(i => !excluding.Contains(i.Name)))
            target.Features.Add(feature with { Target = target });
        return target;
    }

    public static void ConfigureModifiers(this IHasModifiers entity, Action<IModifierBuilder> configure)
